    ...
    def load(self) -> ConfigDict:
        ...
        try:
            ...
            if yaml is not None and isinstance(e, yaml.YAMLError):
                ...
                # Any other error
                logger.error(f"Unexpected error loading config file {self.config_file}: {e}, using defaults")
            self.config = self._get_defaults()
            return self.config
    ...
    def _apply_env_overrides(self):
        ...
        for feature in ['auto_indexing', 'stats_collection', 'expression_checks', 'mutation_logging']:
            ...
            if value in ('false', '0', 'no', 'off'):
                ...
                self._set_nested(f'bypass.features.{feature}.reason', f'Environment variable {env_key}')
    ...
    def _get_defaults(self) -> ConfigDict:
        ...
        return {
            'bypass': {
                ...
                'modules': {
                    'optimization_features': {'enabled': True, 'reason': ''}
                },
                'system': {
                    'enabled': False,
                    'reason': '',
