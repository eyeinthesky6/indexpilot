...
def initialize_tenant_expression(tenant_id):
    ...
    with safe_database_operation(
        "initialize_tenant_expression", f"tenant_{tenant_id}", rollback_on_failure=True
    ) as conn:
        ...
        try:
            ...
            log_audit_event(
                ...
                tenant_id=tenant_id,
                details={
                    "fields_initialized": len(fields),
                    "action": "expression_profile_initialized",
                },
                severity="info",
            )

            print(
                f"Initialized expression profile for tenant {tenant_id} with {len(fields)} fields"
            )
        finally:
            cursor.close()
