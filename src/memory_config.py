...
def calculate_postgres_memory_config() -> dict[str, Any]:
    """
...
def get_postgres_memory_config() -> dict[str, Any]:
    """
...
def generate_postgresql_conf_snippet() -> str:
    ...
    if config['is_windows']:
        lines.append("")
        lines.append("# Windows: maintenance_work_mem limited to 32MB due to shared memory segment limits")
...
def get_memory_status() -> dict[str, Any]:
    ...
    return {
        ...
        "windows_shared_memory_limit_mb": WINDOWS_SHARED_MEMORY_LIMIT_MB if config["is_windows"] else None,
    }
