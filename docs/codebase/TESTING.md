...
python -m pytest tests/test_cli.py tests/test_proposed_index_parser.py tests/test_workload_dna.py tests/test_legacy_index_safety.py -q
...
| Package/CLI | Yes | imports, help, routing, JSON/Markdown/SARIF output | `tests/test_package_surface.py`, `tests/test_cli.py` |
| SQL parser | Yes | read-only queries and constrained proposed indexes | `tests/test_proposed_index_parser.py` |
| Legacy safety | Yes | dry-run lifecycle, disabled cleanup/reindex, factual bloat/write fields | `tests/test_legacy_index_safety.py` |
...
- Current local launch evidence: 165/165 database-independent tests pass. The remaining eight tests
  are explicitly database-backed and report connection refusal when the repository's PostgreSQL
  service is not running on `localhost:5432`.
