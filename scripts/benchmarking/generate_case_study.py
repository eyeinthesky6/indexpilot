...
def load_results():
    ...
    if AUTOINDEX_RESULTS.exists():
        with open(AUTOINDEX_RESULTS) as f:
...
def generate_case_study(name="CRM_Simulation", scenario="small"):
    ...
    improvements = {}
    if (
        baseline_metrics.get("avg_latency")
        and autoindex_metrics.get("avg_latency")
        and baseline_metrics["avg_latency"] > 0
    ):
        improvement = (
            (baseline_metrics["avg_latency"] - autoindex_metrics["avg_latency"])
...
| Index Size | X GB | Y GB | +Z GB | [Details] |"""
    ...
    print("\nMetrics:")
    print(
        f"  Baseline: {baseline_metrics.get('total_queries', 'N/A')} queries, {baseline_metrics.get('avg_latency', 'N/A')}ms avg"
    )
    print(
        f"  Autoindex: {autoindex_metrics.get('total_queries', 'N/A')} queries, {autoindex_metrics.get('avg_latency', 'N/A')}ms avg"
    )
