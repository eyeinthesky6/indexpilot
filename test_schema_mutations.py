...
def test_schema_mutations():
    ...
    try:
        ...
        if result.get('success'):
            ...
            print(f"  - Impact analysis: {len(result.get('impact', {}).get('affected_queries', []))} affected queries")
    ...
    try:
        ...
        print(f"  - Impact: {preview.get('impact', {}).get('affected_queries', 0)} affected queries")
        print(f"  - Rollback SQL: {preview.get('rollback_plan', {}).get('rollback_sql', 'N/A')[:50]}...")
        results['tests_passed'] += 1
    except Exception as e:
        print(f"  ✗ Exception: {e}")
        results['tests_failed'] += 1
    ...
    try:
        ...
        if not result.get('success'):
            print(f"  ✓ System correctly rejected duplicate column: {result.get('error', 'Unknown')}")
            results['tests_passed'] += 1
        else:
